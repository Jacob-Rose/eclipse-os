...
void JacketIO::init()
{
    ...
    //TODO Replace pin #22
    auto [mainStripIt, stripInserted] = strips.emplace(static_cast<uint8_t>(1), make_unique<HSVStrip>(LENGTH, pendant::OutfitLEDPin));
    ...
    if (inserted1) HSVStripNodeFactory::GenerateAxisRow(it1->second.get(), currentPixelIdx, jacket::RING_ONE_LENGTH, Coord(startX1Pos,6.5f), Coord(incrementX1, 0.0f));
    ...
    if (inserted2) HSVStripNodeFactory::GenerateAxisRow(it2->second.get(), currentPixelIdx, jacket::RING_TWO_LENGTH, Coord(startX2Pos,5.0f), Coord(incrementX2, 0.0f));
    ...
    if (inserted3) HSVStripNodeFactory::GenerateAxisRow(it3->second.get(), currentPixelIdx, jacket::RING_THREE_LENGTH, Coord(startX3Pos,4.0f), Coord(incrementX3, 0.0f));
    ...
    if (inserted4) HSVStripNodeFactory::GenerateAxisRow(it4->second.get(), currentPixelIdx, jacket::RING_FOUR_LENGTH, Coord(startX4Pos,3.0f), Coord(incrementX4, 0.0f));
    ...
    if (inserted5) HSVStripNodeFactory::GenerateAxisRow(it5->second.get(), currentPixelIdx, jacket::RING_FIVE_LENGTH, Coord(startX5Pos,2.0f), Coord(incrementX5, 0.0f));
    ...
    if (inserted6) HSVStripNodeFactory::GenerateAxisRow(it6->second.get(), currentPixelIdx, jacket::RING_SIX_LENGTH, Coord(startX6Pos,1.0f), Coord(incrementX6, 0.0f));
    ...
    if (inserted7) HSVStripNodeFactory::GenerateAxisRow(it7->second.get(), currentPixelIdx, jacket::RING_SEVEN_LENGTH, Coord(startX7Pos,0.0f), Coord(incrementX7, 0.0f));
    ...
    if (inserted8) HSVStripNodeFactory::GenerateAxisRow(it8->second.get(), currentPixelIdx, jacket::MONOWIRE_LENGTH, Coord(0.75f,yMonowireOffset), Coord(xShift, yMonowireScalar));
