...
# config/mythos-show.json: the cue list, its layers, and the table of what
# each cue asks of the visualiser. config/mythos26.json is the same room
# opening on the audio meter, for trusting the wire before a set.
#
...
# Launchpad and Synesthesia's analysis arrive, and the picture goes over ssh
# to the scanner-pi, which is where the obelisk's USB cable and the ring's
# GPIO are - `--client`, below. A pi that does not answer is a set on this
# machine's wires with a warning, not a set that does not start.
#
...
# map beside it; --midimap overrides. Empty string loads the viewer's default
# (midimaps/default.json beside the config) instead. The all-states map, every
# machine with a tab each, is the same tool's other output.
...
# trade from --host, and the reason it is the default is MIDI - the
# executable is what opens a port, so under --host the beat from Mixxx and
# every pad on the Launchpad belong to the pi and this desk has neither.
# Under --client they stay here, where they are plugged in, and the frames go
# over instead. So does the show itself: the pi only paints, and a binary
# there that has never heard of tonight's cues is not a reason its wires
# stay dark (see "Client mode" in the readme).
#
...
#
# `-` rather than `:-`, so an exported empty ECLIPSE_CLIENT means "this
# machine's wires" the same way --local does; unset means the pi.
CLIENT="${ECLIPSE_CLIENT-$DEFAULT_HOST}"

# Whether the pi was *asked for* rather than defaulted. Named, it is believed:
# a pi that does not answer is a set that will not start, said loudly. Defaulted,
# it is a preference, and the set runs on this machine's wires instead.
client_named=0
...
launch-mythos-set.sh - the mythos26 set, on this machine
  ...
  --midimap FILE       the map the pads and lamps run off (default the show's
                       own, midimaps/<config name>.json beside the config -
                       config/midimaps/mythos-show.json - or $ECLIPSE_MIDIMAP;
                       generated by tools/make-launchpad-map.py). Empty string
                       takes the viewer's own default, midimaps/default.json.
  --client [NAME]      run the show HERE and put the frames on NAME's wires
                       (the default: scanner-pi, or $ECLIPSE_CLIENT). Mixxx
                       and the Launchpad keep working, because they never
                       leave this machine; the rig's picture crosses the
                       network instead of its cues. Defaulted, a pi that
                       does not answer means the set runs on this machine's
                       wires; named, it means the set does not start.
  --local              the wires are on this machine: no pi, no ssh. What
                       --bench does to the wires, this does to the network.
  --host [NAME]        the other way round: run the show on another machine
                       over ssh, keeping the window here (default scanner-pi,
                       or $ECLIPSE_HOST). The wires are the host's, and so is
                       MIDI: the pads and lamps default to off unless
                       --midi/--midi-out are named, because the controller
                       plugged in here is invisible to a show over there.
  --config PATH        a different show (default config/mythos-show.json)
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --midimap)         MIDIMAP="${2?--midimap needs a file (or '')}"; midimap_named=1; shift ;;
...
# The map follows the config unless one was named: config/foo.json runs on
# config/midimaps/foo.json. Resolved after the options so --config moves it.
# A config with no map of its own - mythos26.json, the meter - gets the
# all-states map, every machine with a tab each, which is the surface that
# makes sense for a rig that is not on a cue list.
if [ -z "${ECLIPSE_MIDIMAP+x}" ] && [ "$midimap_named" = 0 ]; then
    MIDIMAP="$(dirname "$CONFIG")/midimaps/$(basename "${CONFIG%.*}").json"
    if [ ! -f "$MIDIMAP" ]; then
        echo "no map of its own for $CONFIG ($MIDIMAP); using the all-states map" >&2
        MIDIMAP="$(dirname "$CONFIG")/midimaps/launchpad-all-states.json"
    fi
fi
...
# --host on its own means the pi the other way round, not both ways at once:
# the defaulted client steps aside for it. Two *named* ends are a real
# contradiction.
if [ -n "$HOST" ] && [ -n "$CLIENT" ] && [ "$client_named" = 0 ]; then
    CLIENT=""
fi
...
if [ -n "$REMOTE" ]; then
    ...
    if ! ssh -o BatchMode=yes -o ConnectTimeout=5 "$REMOTE" true 2>/dev/null; then
        if [ -n "$CLIENT" ] && [ "$client_named" = 0 ]; then
            # The pi was a default, not a request. The same rule as a
            # controller left in the flight case: it must not be the reason a
            # set does not run. Said plainly, because from across a room a
            # dark sculpture and a pi that never answered look the same.
            echo "warning: '$CLIENT' is not answering ssh - running on this machine's" >&2
            echo "         wires instead. The pi's obelisk and ring will stay dark." >&2
            echo "         --client $CLIENT to insist; --local to stop asking." >&2
            CLIENT=""
            REMOTE=""
        else
            if [ -n "$CLIENT" ]; then
                echo "warning: 'ssh $REMOTE true' did not succeed - the set will run, and the rig on $REMOTE will stay dark." >&2
            else
