...
// CREDIT
// ----------------------------------------------------------------------------
// This is Felix Woitzel's "Nova", the scene that ships inside Synesthesia
// (Synesthesia 1.25.4, Contents/Resources/release/nova.synScene), itself
// grown from his ShaderToy work. The feedback buffers, the mip-pyramid blur,
...
//
// The original paints with six named colours - a base wash, the galaxies, an
// add/subtract tint, a bleed, the fine structure of the radial grid, and a
// depth shade under the galaxies - and has four hard-coded sets of them, its
// "colour regimes". Which regime you get is chosen at random each time the
// scene is transitioned to (SMOOTH_TRANSITIONS on cReg0..cReg3 in the
// original scene.json) and cross-faded over three seconds. So the root colour
// of this look is not a function of anything - not the music, not the clock -
// it is one of four constants, drawn by lot.
//
// This copy keeps the six roles and derives all of them from the rig:
//
...
//
//   rig_color_2  the SECOND, /controls/global/color/2. Nothing sends this yet
//                (the sender is one fixture), so by default it is not read:
//                `auto_second` derives the second colour as the complement
//                of the first, which is what the original's regime 1 pairs
//                (orange galaxies on a sky-blue wash) and close enough to
//                regime 0 (teal on yellow). Turn `auto_second` off to drive
//                both colours independently.
//
// How the roles are filled from two colours - see rigPalette() - is a rule,
// stated once, so that a colour change on the truss changes the whole picture
// coherently instead of one part of it. The original regimes are listed in
// the readme for anyone who wants an eclipse pattern to match one of them
// exactly instead.
...
vec2 vortex_pair_warp(vec2 uv, vec2 pos, vec2 vel)
{
  ...
  vec4 loadMediaZoomed(vec2 offset, float zoom) {
    vec2 uv = _uv;
    uv *= zoom;
    uv -= 1/zoom;
    
    vec4 tempCol = _textureMediaAsMask(uv+offset);
		
    if (uv.x < 0.0 || uv.y < 0.0 || uv.y > 1.0 || uv.x > 1.0) {
      return _applyInvertMedia(vec4(0.0));
    }
...
void mainImage0(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  if (_isMediaActive()){
    vec4 tempCol = loadMediaZoomed(vec2(0.0),2.0);
...
// ---- ECLIPSE: where the colours come from -----------------------------------

struct Palette
{
    vec3 base;           // the wash behind everything
    vec3 galaxy;         // the galaxies / neurons - the colour you would name
    vec3 subAdd;         // tint added or subtracted along the flow gradients
    vec3 bleed;          // what the darkness bleeds into, away from the galaxies
    vec3 fineStructure;  // the radial grid, when it is on
    vec3 depth;          // the shade under the galaxies that gives them depth
};

/// The same hue on the other side of the wheel; a grey stays grey.
vec3 complementOf(vec3 rgb)
{
    vec3 hsv = _rgb2hsv(rgb);
    return _hsv2rgb(vec3(fract(hsv.x + 0.5), hsv.y, hsv.z));
}

/// Two colours in, six roles out.
///
/// The first colour is the galaxies, because that is the part of the picture
/// a person points at. The second is the wash they sit on, and it is either
/// sent (rig_color_2) or, by default, derived as the first's complement - the
/// original's regime 1 is exactly that pairing. Everything else is a mix of
/// the two: the bleed is the galaxy colour lightened, the fine structure is
/// the wash lightened, the add/subtract tint and the depth sit between them.
/// One rule, so a colour change on the truss moves the whole picture.
Palette rigPalette()
{
    ...
    Palette p;
    p.galaxy = first;
    p.base = second * base_amount;
    p.bleed = mix(first, vec3(1.0), 0.4);
    p.fineStructure = mix(second, vec3(1.0), 0.5);
    p.subAdd = mix(first, second, 0.5) * 0.7;
    p.depth = mix(first, second, 0.35);
    return p;
}
...
void mainImage4(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  // ---- ECLIPSE: the palette ------------------------------------------
  // The original declares the six colours here as constants and then
  // cross-fades each towards three alternative sets by cReg1..cReg3 (the
  // regimes; see the readme for their values). Here all six come from the
  // rig, by the rule in rigPalette(). Same names, same uses below.
  Palette pal = rigPalette();
  vec3 baseCol = pal.base;
  vec3 galaxyCol = pal.galaxy;
  vec3 subAddColor = pal.subAdd;
  vec3 bleed = pal.bleed;
  vec3 fineStructure = pal.fineStructure;
  vec3 depthGalaxies = pal.depth;
  // ---- end ECLIPSE ----------------------------------------------------
