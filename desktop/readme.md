...
| 14 `clouds` | a sunset down the stage — yellow overhead into pink at the horizon — over a cloud deck in shadow, streaming past; `height` and `speed` glide to where four pads put them | *Cloud Ten* + `white.mp4`, with its auto height off; the same four pads ramp its `manual_height` and `speed`. Checked against the scene — white, not black, because this one multiplies by its media rather than tinting with it |
