...
| 12 `churn` | the neuron's field in the churn's two colours — red into blue — busier, pushed by the level the way the paint is, and its `contrast` up so a patch picks a side far more often than it sits purple between; three palettes and a rainbow on the mode pads | *Eclipse Churn* + `black.mp4`; the probe sends colour A, the cue table colour B |
